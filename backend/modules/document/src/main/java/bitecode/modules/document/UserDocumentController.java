...
public class UserDocumentController {
    ...
    public PagedModel<DocumentDetails> getDocuments(Pageable pageable) {
        return new PagedModel<>(documentService.findUserDocuments(AuthUtils.getUserId(), pageable));
    }

    @GetMapping("/{documentId}")
    public DocumentDetails getDocument(@PathVariable UUID documentId) {
        return documentService.findDocumentForUser(documentId, AuthUtils.getUserId());
    }
